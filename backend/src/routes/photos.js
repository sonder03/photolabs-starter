...
module.exports = db => {
  ...
  router.get("/photos/:searchTerm", (request, response) => {
    ...
    db.query(`
      ...
      WHERE (photo.city like '%${request.params.searchTerm}%'  
            or photo.country like '%${request.params.searchTerm}%'  
            or user_account.fullname like '%${request.params.searchTerm}%'  
            or user_account.username like '%${request.params.searchTerm}%'  
            );
    `).then(({ rows }) => {
